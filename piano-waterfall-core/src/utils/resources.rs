...
fn xdg_config() -> Option<PathBuf> {
    env::var_os("XDG_CONFIG_HOME")
        .filter(|h| !h.is_empty())
        .map(PathBuf::from)
        .map(|p| p.join("piano_waterfall"))
        .or_else(|| home().map(|h| h.join(".config").join("piano_waterfall")))
}
...
pub fn default_sf2() -> Option<PathBuf> {
    #[cfg(all(target_family = "unix", not(target_os = "macos")))]
    {
        ...
        // <prefix>/bin/piano_waterfall -> <prefix>/share/piano_waterfall/default.sf2
        ...
        let flatpak = PathBuf::from("/app/share/piano_waterfall/default.sf2");
        if flatpak.exists() {
            return Some(flatpak);
        }
