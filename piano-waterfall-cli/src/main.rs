...
impl Recorder {
    ...
    fn update(&mut self, delta: Duration) {
        ...
        if let Some(note_labels) = self.note_labels.as_mut() {
            note_labels.update(
                piano_waterfall_core::dpi::PhysicalSize {
                    width: self.width,
                    height: self.height,
                },
                1.0,
        ...
        self.text.update(
            piano_waterfall_core::dpi::PhysicalSize::new(self.width, self.height),
            1.0,
        );
    }
...
// TODO: This is copy-pasted from piano_waterfall/output_manager/synth_backend
