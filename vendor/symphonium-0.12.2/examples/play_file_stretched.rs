...
use symphonium::{DecodedAudio, cache::SymphoniumCache};
...
pub fn main() {
    ...
    if args.len() == 1 {
        println!(
            "usage: cargo run --example play_file_stretched <ratio> <path-to-audio-file>\ne.g. cargo run --example play_file_stretched 0.75 test_files/synth_keys_44100.ogg"
        );
        return;
    }
    ...
    let ratio = args[1]
        .parse::<f64>()
        .expect(&format!("invalid ratio: {}", &args[1]));
    ...
    let probed = symphonium::probe_from_file(
        file_path,
        // The codec prober. Set to `None` to use the default one from symphonia.
        None,
    )
    .unwrap();

    let audio_data: DecodedAudio = symphonium::decode_stretched(
        probed,
        ratio,
        Some(sample_rate),   // target_sample_rate
        &Default::default(), // config
        Some(&cache),
        // The codec registry. Set to `None` to use the default one from symphonia.
        None,
    )
    .unwrap()
    .into();
