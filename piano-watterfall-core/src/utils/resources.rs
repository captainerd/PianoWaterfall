...
fn xdg_config() -> Option<PathBuf> {
    env::var_os("XDG_CONFIG_HOME")
        .filter(|h| !h.is_empty())
        .map(PathBuf::from)
        .map(|p| p.join("piano_watterfall"))
        .or_else(|| home().map(|h| h.join(".config").join("piano_watterfall")))
}
...
pub fn default_sf2() -> Option<PathBuf> {
    #[cfg(all(target_family = "unix", not(target_os = "macos")))]
    {
        ...
        // <prefix>/bin/piano_watterfall -> <prefix>/share/piano_watterfall/default.sf2
        if let Some(path) = std::env::current_exe().ok().and_then(|exe_path| {
            exe_path
                .parent()
                .and_then(|path| path.parent())
                .map(|pfx_path| pfx_path.join("share").join("piano_watterfall").join("default.sf2"))
        }) && path.exists()
        {
            return Some(path);
        }

        let flatpak = PathBuf::from("/app/share/piano_watterfall/default.sf2");
        if flatpak.exists() {
            return Some(flatpak);
        }
