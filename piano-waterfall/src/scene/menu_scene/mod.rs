...
use piano_waterfall_core::render::{BgPipeline, ImageIdentifier, QuadRenderer, TextRenderer};
...
impl MenuScene {
    pub fn new(ctx: &mut Context, song: Option<Song>) -> Self {
        ...
        let logo = nuon_renderer.add_image(piano_waterfall_core::render::Image::new(
            &ctx.gpu.device,
            &ctx.gpu.queue,
            logo,
        ));
